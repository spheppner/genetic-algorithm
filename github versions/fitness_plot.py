...
if __name__ == "__main__":
    ...
    for gen_idx in range(n_generations):
        fittest = []
        fitness_list = [c._get_fitness() for c in population]
        fitness_over_time.append(round(sum(fitness_list)/len(fitness_list),3))
        fittest.append(population[fitness_list.index(max(fitness_list))])
        fitness_list.remove(max(fitness_list))
        fittest.append(population[fitness_list.index(max(fitness_list))])

        population = []
        for pop_idx in range(n_population):
            population.append(fittest[0].mate(fittest[1]))
